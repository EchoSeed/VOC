...
// ── Truncation detector ─────────────────────────────────────
// A source is likely truncated if it ends mid-sentence:
// no terminal punctuation, ends mid-word, or cuts off a known pattern
function detectTruncation(text) {
  const t = text.trimEnd();
  const lastChar = t[t.length - 1];
  const terminalPunct = [".", "!", "?", "}", "]", '"', "'", "`"];
  const midSentence = !terminalPunct.includes(lastChar);
  // Also flag if last word looks incomplete (no vowel, very short after last space)
  const lastWord = t.split(/\s+/).pop() || "";
  const looksIncomplete = lastWord.length < 3 && /^[a-z]/i.test(lastWord);
  return midSentence || looksIncomplete;
}
...
export default function PhilosophersStone() {
  ...
  const runPipeline = useCallback(async () => {
    ...
    // ── Pre-flight checks ─────────────────────────────────
    const truncated = detectTruncation(input);
    if (truncated) {
      addLog("⚠️ Source appears truncated (no terminal punctuation) — fidelity gap likely at end of source", "error");
    }
