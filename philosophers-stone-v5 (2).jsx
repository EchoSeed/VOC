    ...
    try {
      ...
      const tripleP = selected.has("triple_mode") && all.concepts.length ? (async () => {
        ...
        try {
          ...
          // ── Elaboration inflation check ───────────────────
          const minWords = Math.max(DP.wordCount, 100); // floor at 100 so short sources aren't over-penalized
          const elabRatio = all.elaborations.length / Math.max(minWords / 50, 1);
          if (elabRatio > 3) {
            addLog(`⚠️ Elaboration inflation: ${all.elaborations.length} elaborations against ~${DP.wordCount}-word source (ratio ${elabRatio.toFixed(1)}x) — fidelity risk`, "error");
          }
      ...
      const compressP = selected.has("compress_expand") && all.concepts.length ? (async () => {
        ...
        try {
          // Cap concepts sent to prompt — glossary generation is the bottleneck
          const compressConcepts = toArr(all.concepts)
            .sort((a, b) => (b.importance || 0) - (a.importance || 0))
            .slice(0, Math.min(all.concepts.length, 17));
      ...
      await Promise.all([substantiationP, tripleP, semP, compressP]);
      ...
      const forgeP = selected.has("skill_forge") && all.concepts.length ? (async () => {
        ...
        try {
          ...
          const conceptRows = concepts.map(c => {
            ...
            const cite = toStr(c.source_citation).replace(/\|/g, "&#124;").slice(0, 80);
