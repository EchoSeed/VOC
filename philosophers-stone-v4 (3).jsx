...
//  EchoSeed v4.2 — Refactored
// ═══════════════════════════════════════════════════════════════

const DP = { concepts: 41, terms: 29, clusters: 13, glossary: 37, thesis: 11 };
...
const PROMPTS = {
  dual_extract: (text) => ({
    ...
    user: `Analyze this text and extract exactly ${DP.concepts} concepts. For each: technical definition (max 80 words), plain explanation (max 60 words), importance 0.0-1.0.\n\nTEXT:\n${text.slice(0, 4000)}\n\nReturn ONLY: [{"id":1,"concept":"...","technical":"...","plain":"...","importance":0.9}]`,
  }),

  triple_mode: (text, concepts) => ({
    system: "Output ONLY a raw JSON array. Each element: {concept_id, concept, decision, analogy, insight}. Max 100 words per field. No prose, no fences.",
  ...
  skill_forge: (text, all) => {
    ...
    return {
      system: "You are a Skill File compiler. Output two parts: first a raw JSON object for metadata (no fences), then Python code blocks wrapped in exact <<<SENTINEL>>> tags. No markdown fences anywhere. No extra prose.",
    ...
    <div>
      ...
      {items.map((c, i) => (
        <div key={i} style={{ marginBottom: 10, padding: "8px 10px", borderRadius: 6, background: "rgba(255,255,255,0.02)", borderLeft: `3px solid ${color}` }}>
          ...
          <div style={{ fontSize: 10, color: "#22d3ee", marginBottom: 3, lineHeight: 1.4 }}><span style={{ fontWeight: 600, color: "#22d3ee99" }}>Technical:</span> {toStr(c.technical)}</div>
...
export default function PhilosophersStone() {
  ...
  const runPipeline = useCallback(async () => {
    ...
    setStatus(st);
    addLog(`Pipeline initiated — ${DP.concepts} concepts · ${DP.terms} terms · ${DP.clusters} clusters`);
    ...
    try {
      ...
      if (selected.has("dual_extract")) {
        ...
        try {
          const pr = PROMPTS.dual_extract(input);
      ...
      const semP = selected.has("semantic_index") && all.concepts.length ? (async () => {
        ...
        try {
          const pr = PROMPTS.semantic_index(input, all.concepts);
          const raw = await callAPI(pr.system, pr.user, 4000);
      ...
      const compressP = selected.has("compress_expand") && all.concepts.length ? (async () => {
        ...
        try {
          const pr = PROMPTS.compress_expand(input, all.concepts);
          const raw = await callAPI(pr.system, pr.user, 4000);
      ...
      const metaP = selected.has("meta_analysis") ? (async () => {
        ...
        try {
          const pr = PROMPTS.meta_analysis(input, all);
          const raw = await callAPI(pr.system, pr.user, 4000);
      ...
      const forgeP = selected.has("skill_forge") && all.concepts.length ? (async () => {
        ...
        try {
          const pr = PROMPTS.skill_forge(input, all);
          const raw = await callAPI(pr.system, pr.user, 4000);
          ...
          const conceptRows = concepts.map(c => {
            ...
            return `| ${toStr(c.concept)} | ${toStr(c.technical).replace(/\|/g, "&#124;").slice(0, 80)} | ${toStr(c.plain).replace(/\|/g, "&#124;").slice(0, 80)} | ${imp} |`;
          }).join("\n");

          const glossaryRows = glossary.map(g =>
            `| ${toStr(g.term)} | ${toStr(g.definition).replace(/\|/g, "&#124;").slice(0, 100)} | ${(g.concept_ids || []).join(", ")} |`
          ).join("\n");
      ...
      <div style={S.header}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <p style={S.sub}>v4 — AI Agent Markdown Forge · EchoSeed</p>
          </div>
          {running && (
