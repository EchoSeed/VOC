...
// ── Truncation detector ──────────────────────────────────────
// Returns { truncated: bool, confidence: 'high'|'low', reason: string }
function detectTruncation(text) {
  const t = text.trimEnd();
  if (!t) return { truncated: false, confidence: "low", reason: "" };
  const lastChar = t[t.length - 1];
  const lastLine = t.split("\n").pop().trimEnd();
  const lastWord = t.split(/\s+/).pop() || "";

  // High-confidence truncation signals
  const highSignals = [
    // Ends mid-word (letter/digit, not punctuation)
    { test: /[a-zA-Z0-9]$/.test(lastChar) && !["s","d","n","g","e","r","t","y","l","h"].includes(lastChar), reason: "ends mid-word" },
    // Ends with comma or semicolon — always mid-sentence
    { test: [",", ";"].includes(lastChar), reason: "ends with comma/semicolon" },
    // Ends with opening bracket/paren not closed
    { test: ["(", "[", "{"].includes(lastChar), reason: "ends with unclosed bracket" },
    // Ends with a colon — implies continuation
    { test: lastChar === ":" && !lastLine.endsWith('"""') && !lastLine.endsWith("'''"), reason: "ends with colon" },
    // Docstring or comment cut mid-phrase (odd number of triple-quote markers)
    { test: (() => { const tq = (t.match(/"""/g) || []).length; return tq % 2 !== 0; })(), reason: "unclosed docstring" },
  ];

  // Low-confidence signals (flag but don't penalize hard)
  const lowSignals = [
    { test: lastWord.length < 3 && /^[a-z]/.test(lastWord) && !/^(of|in|is|at|on|by|to|as|or|if|it|an|no|so|do|go|be|we|he|me|my|us)$/.test(lastWord), reason: "suspiciously short last word" },
    // Source ends mid-identifier (camelCase or snake_case fragment)
    { test: /[a-z]_$/.test(lastWord) || /[a-z][A-Z][a-z]*$/.test(lastWord.slice(-5)), reason: "ends mid-identifier" },
  ];

  // Code-safe terminal chars — these are NOT truncation
  const codeTerminals = [")", "}", "]", '"', "'", "`", "/", "*", "\\", "#"];
  if (codeTerminals.includes(lastChar)) {
    // Still check for the high-confidence structural signals
    const highHit = highSignals.find(s => s.test);
    if (highHit) return { truncated: true, confidence: "high", reason: highHit.reason };
    return { truncated: false, confidence: "high", reason: "ends with code terminal" };
  }

  const highHit = highSignals.find(s => s.test);
  if (highHit) return { truncated: true, confidence: "high", reason: highHit.reason };

  const lowHit = lowSignals.find(s => s.test);
  if (lowHit) return { truncated: true, confidence: "low", reason: lowHit.reason };

  return { truncated: false, confidence: "high", reason: "" };
}

// ── Source density classifier ─────────────────────────────
// Documentation and code are sparse — same word count carries fewer inferential claims
// than argument prose. Apply a density coefficient before scaling.
function classifyDensity(text) {
  const codeSignals   = (text.match(/def |class |return |import |->|:$/gm) || []).length;
  const docSignals    = (text.match(/param|arg|returns?|raises?|:$/gm) || []).length;
  const argueSignals  = (text.match(/therefore|because|implies?|suggests?|however|thus/gi) || []).length;
  if (codeSignals > 5)  return { type: "code",          coefficient: 0.35 };
  if (docSignals > 3)   return { type: "documentation", coefficient: 0.45 };
  if (argueSignals > 2) return { type: "argument",      coefficient: 0.85 };
  return                       { type: "prose",          coefficient: 0.65 };
}

// ── Source-proportional prime scaling ────────────────────────
function computeDP(text) {
  const words = text.trim().split(/\s+/).length;
  const { type, coefficient } = classifyDensity(text);
  // Apply density coefficient — documentation needs fewer concepts per word
  const rawConcepts = Math.round(words * coefficient / 8);
...
const STAGES = [
  { id: "code_preprocess", name: "Code Preprocessor",         icon: "🧹",  deps: [],                                                   color: "#64748b", desc: "Strip syntax noise — extract semantic payload from code", optional: true },
  { id: "dual_extract",    name: "Dual-Register Extraction", icon: "⚗️",  deps: [],                                                   color: "#f59e0b", desc: "Technical + plain language concept pairs" },
  { id: "substantiation",  name: "Substantiation",           icon: "🔎",  deps: ["dual_extract"],                                     color: "#818cf8", desc: "Pre + post hoc rationale · Provenance verification · Coverage gaps" },
...
async function callAPI(system, user, maxTok = 4000, retries = 1) {
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      const delay = e.message?.includes('429') ? 8000 : 1500;
...
const PROMPTS = {
  code_preprocess: (text) => ({
    system: "You are a semantic extractor for source code. Output ONLY the extracted semantic content as clean plain text — no code syntax, no fences.",
    user: `Extract the semantic payload from this source code. Output only:
- Class and function names with their purpose (from docstrings or inferred from name)
- Parameter names and what they represent (semantic meaning, not types)
- Key variable names and their role
- Inline comments verbatim
- Docstring content verbatim
- Conditional logic described in plain language (e.g. "if completion > 0.8 and causal < 0.2, flag as zero utility")
- Return value semantics

Do NOT output: import statements, type annotations, decorators, default values, syntax keywords, brackets, or boilerplate.
...
Output clean plain text only:`,
  ...
  dual_extract: (text, DP) => ({
    system: `You are a citation-grounded concept extractor. Every concept MUST cite verbatim source text as evidence. Output ONLY a raw JSON array, no prose, no fences.`,
    user: `Extract up to ${DP.concepts} concepts from this source. Each concept requires a verbatim source_citation proving it exists in the text.

SCHEMA (every field required):
{
  "id": 1,
  "concept": "short name",
  "source_citation": "verbatim phrase copied from source text — if you cannot find one, do not include this concept",
  "technical": "extracted: OR inferred: prefix + max 25 words matching source language",
  "plain": "max 20 words",
  "importance": 0.0-1.0
}

Hard rules:
- source_citation MUST be a direct quote from the source — paraphrase is not acceptable
- technical MUST start with "extracted:" (citation present, source vocabulary) or "inferred:" (no direct source phrase)
- If source_citation is empty or paraphrased, mark technical as "inferred:" regardless
- DIRECTIONALITY: preserve directional qualifiers verbatim — "may drift upward" means upward only
- CONSTRUCT SEPARATION: one source name = one concept; do not merge causal_contribution_fn and proxy_metric_fn
- PROCEDURAL: extract docstring Usage blocks as a concept; cite the usage lines verbatim as source_citation
- INTERVALS: if source mentions histories or intervals more than once, extract audit-interval cadence as a concept
- TRUNCATION: if source ends mid-sentence, include concept "source-truncation-gap" with source_citation = last complete phrase before cutoff and importance 0.95
- TRUNCATION MID-FIELD: if a docstring, comment, or description ends mid-phrase inside the source (not just at the end), flag that specific field as a source artifact defect by including source_citation = the truncated phrase and marking technical as "extracted: definition truncated mid-phrase — downstream meaning is inferred"
- INLINE COMMENTS AND VARIABLE NAMES: inline comments (# ...) and named intermediate variables carry implementation-level specificity — extract them as concepts with their comment text or variable name as source_citation; do not drop them as boilerplate
- BOOLEAN FIELD COMBINATIONS: if a source defines two or more boolean fields on the same construct (e.g. accessible and detectable), check whether their combination implies a risk ranking or decision matrix not stated in the source — if so, extract the combination as a separate concept prefixed "inferred:" and flag it explicitly rather than embedding it silently in elaborations
- Do NOT invent concepts with no source anchor — fewer accurate concepts > more inflated ones
...
Return ONLY the JSON array.`,
  ...
  substantiation: (text, concepts) => ({
    system: "You are a provenance auditor. For each extracted concept, generate pre-hoc and post-hoc rationale that strictly obeys source provenance. Output ONLY a raw JSON array, no fences, no prose.",
    user: `SOURCE:
${text.slice(0, 4000)}

EXTRACTED CONCEPTS:
${JSON.stringify(concepts.map(c => ({ id: c.id, concept: c.concept, technical: c.technical, source_citation: c.source_citation || "" })))}

For each concept generate:
- pre_hoc: why this concept warranted extraction — what specific source language, structure, or implication licensed it (max 60 words, must cite source vocabulary)
- post_hoc: what the extraction contributes to understanding the source — and whether it holds under provenance scrutiny (max 60 words)
- verdict: "sound" (citation present, technical matches source), "weak" (citation thin or paraphrased), or "unsound" (no source anchor, should be inferred: or removed)
- coverage_gap: if analyzing this concept reveals a RELATED concept that was missed and IS directly in the source, name it here; otherwise null

Rules:
- pre_hoc must quote or closely paraphrase actual source text — no domain imports
- post_hoc must not introduce new claims beyond what pre_hoc establishes
- verdict "unsound" means the concept should be demoted to inferred: or removed from the concept list
- coverage_gap must be directly source-stated, not inferred

Return ONLY: [{"id":1,"concept":"name","pre_hoc":"...","post_hoc":"...","verdict":"sound|weak|unsound","coverage_gap":"concept name or null"}]`,
  }),
...
Return ONLY: [{"concept_id":1,"concept":"name","provenance":"extracted","decision":"...","analogy":"...","insight":"..."}]

CRITICAL: Every element MUST include "provenance" set to either "extracted" or "inferred" — no exceptions.`,
  }),
...
Return ONLY: {"terms":[{"term":"...","concept_ids":[1,2]}],"clusters":[{"name":"...","concept_ids":[1,2],"description":"..."}],"density_map":{"highest":"...","lowest":"...","avg_connections":2.5}}`,
  ...
  compress_expand: (text, concepts, DP) => ({
    system: `Compress knowledge into a ${DP.thesis}-sentence thesis and up to ${Math.min(DP.glossary, 13)}-entry glossary. Output ONLY a raw JSON object, no fences.`,
    user: `CONCEPTS:
...
- DIRECTIONALITY: preserve directional qualifiers from source (e.g. "upward drift only") — do not generalize unless source warrants it
- CONSTRUCT SEPARATION: if the source names two constructs separately, the thesis must treat them separately — do not collapse causal_contribution_fn and proxy_metric_fn or similar pairs into one concept
...
Return ONLY: {"thesis":"...","glossary":[{"term":"...","definition":"...","concept_ids":[1]}]}`,
  ...
  meta_analysis: (text, all, DP) => ({
    system: "You are a rigorous pipeline auditor with strict audit fidelity. Score HIGH when the pipeline captured the source thoroughly. Penalize conflation of inferred theoretical extensions with directly extracted content. CRITICAL: before claiming a concept is absent from the source, verify it against the full SOURCE text provided — do not fabricate absence claims. Output ONLY a raw JSON object, no fences. emergence_assessment must be a plain string.",
    user: `SOURCE (~${text.trim().split(/\s+/).length} words)${all.truncated ? ` [TRUNCATED — ${all.truncated.confidence} confidence: ${all.truncated.reason}]` : ""}:\n${text.slice(0, 4000)}\n\nCONCEPTS EXTRACTED: ${toArr(all.concepts).length} (target was ${DP.concepts})\nINFERRED CONCEPTS (by provenance field): ${toArr(all.elaborations).filter(e => e.provenance === "inferred").length} elaborations | ${toArr(all.concepts).filter(c => toStr(c.technical).startsWith("inferred:")).length} concepts\nTAXONOMY TERMS: ${all.taxonomy?.terms?.length || 0} | CLUSTERS: ${all.taxonomy?.clusters?.length || 0}\nTHESIS: ${toStr(all.compression?.thesis)}\nELABORATIONS: ${toArr(all.elaborations).length}\n\nSource-fidelity audit rules:\n- Penalize concepts that import domain terminology not in source (reward hacking, principal-agent, tractability bias, etc.)\n- Penalize taxonomy terms that cannot be traced to source language or clear implication\n- Penalize over-expansion of sparse governance signals (e.g. "left unchecked") into full frameworks\n- Reward accurate provenance tagging (extracted: vs inferred:)\n- coverage_score reflects source fidelity, not concept volume — fewer accurate concepts scores higher than many inflated ones\n\nREFLEXIVE PASS — check whether the source's own framing, metadata, structural context, or authorship bears on any extracted concept in a way the pipeline has not surfaced:\n- Does the source's format, speaker identity, or document structure contradict or enrich any extracted concept?\n- Is there a self-referential detail (e.g. the narrator enacting the behavior being described, the medium contradicting the message) that strengthens or undermines the thesis without requiring inference?\n- Does any extracted concept apply to the source itself as an instance of what it describes?\n- Is the source document structured as a specification, API reference, or scaffolding artifact? If so, does it occupy the same functional role as the constructs it describes — making it itself an instance of what it specifies?\n- If yes, surface it in reflexive_observations as a direct citation, not an elaboration\n\nReturn ONLY: {"complexity_profile":{"conceptual_density":0.0,"technical_depth":0.0,"abstraction_level":0.0,"interconnectedness":0.0},"emergence_assessment":"...","blind_spots":["..."],"coverage_score":0.0,"recommendations":["..."],"reflexive_observations":["direct citation of self-referential detail if present, else empty array"]}`,
  }),
  ...
  skill_forge: (text, all, DP) => {
    ...
    const reflexive = toArr(all.meta?.reflexive_observations);
    const blindSpots = toArr(all.meta?.blind_spots);
    const recommendations = toArr(all.meta?.recommendations);
    const prohibitedTerms = toArr(all.meta?._prohibited_terms);

    return {
      system: `You are a Skill File compiler with strict source fidelity. Output two parts: first a raw JSON object for metadata (no fences), then code blocks wrapped in exact <<<SENTINEL>>> tags. No markdown fences. No extra prose. Do not present inferred theoretical extensions as extracted facts.${prohibitedTerms.length ? " PROHIBITED TERMS (meta-analysis flagged these as pipeline vocabulary not in source — do not use them): " + prohibitedTerms.join(", ") + "." : ""}`,
...
${DP.sourceType === "code" ? `<<<IMPLEMENTATION>>>
...
<<<END_CHEATSHEET>>>` : `<<<IMPLEMENTATION>>>
Pseudocode / Decision Logic:
1. [entry condition or trigger]
2. [core operation — use exact source vocabulary]
3. [branch conditions if present]
4. [output or side effect]
Note: use source-attested terms only; mark any extension as [inferred]
<<<END_IMPLEMENTATION>>>

<<<CHEATSHEET>>>
Quick-Reference Pattern:
- When: [trigger condition from source]
- Do: [action from source]
- Check: [verification step from source]
- Avoid: [failure mode from source]
<<<END_CHEATSHEET>>>`}`,
    };
  },
};
...
function ReportCodePreprocess({ data, color }) {
  const txt = typeof data === "string" ? data : "";
  if (!txt) return <div style={{ color: "#555", fontSize: 11 }}>No preprocessed output</div>;
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 700, color, marginBottom: 8 }}>🧹 Semantic Payload Extracted</div>
      <div style={{ fontSize: 10, color: "#64748b", marginBottom: 6 }}>{txt.length} chars — feeding into dual extract</div>
      <pre style={{ whiteSpace: "pre-wrap", wordBreak: "break-word", fontSize: 10, lineHeight: 1.6, color: "#94a3b8", background: "rgba(0,0,0,0.3)", padding: 10, borderRadius: 6, margin: 0, maxHeight: 300, overflow: "auto" }}>{txt}</pre>
    </div>
  );
}
      ...
      {items.map((c, i) => (
        <div key={i} style={{ marginBottom: 10, padding: "8px 10px", borderRadius: 6, background: "rgba(255,255,255,0.02)", borderLeft: `3px solid ${color}` }}>
          ...
          <div style={{ fontSize: 10, color: "#22d3ee", marginBottom: 3, lineHeight: 1.4 }}>
            ...
            {toStr(c.technical).startsWith("inferred:") && <Badge color="#f472b6">inferred</Badge>}
            {toStr(c.technical).startsWith("extracted:") && <Badge color="#34d399">extracted</Badge>}
          </div>
          {c.source_citation && (
            <div style={{ fontSize: 9, color: "#475569", marginBottom: 3, lineHeight: 1.4, fontStyle: "italic", borderLeft: "2px solid #334155", paddingLeft: 6 }}>
              "{toStr(c.source_citation)}"
            </div>
          )}
...
function ReportSubstantiation({ data, color }) {
  const items = toArr(data);
  if (!items.length) return <div style={{ color: "#555", fontSize: 11 }}>No substantiation data</div>;
  const verdictColor = v => v === "sound" ? "#34d399" : v === "weak" ? "#f59e0b" : "#ef4444";
  const gaps = items.filter(i => i.coverage_gap && i.coverage_gap !== "null");
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 700, color, marginBottom: 8 }}>🔎 {items.length} Concepts Substantiated</div>
      {gaps.length > 0 && (
        <div style={{ marginBottom: 10, padding: "8px 10px", borderRadius: 6, background: "rgba(129,140,248,0.08)", border: "1px solid rgba(129,140,248,0.3)" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#818cf8", marginBottom: 4 }}>📍 COVERAGE GAPS DETECTED ({gaps.length})</div>
          {gaps.map((g, i) => <div key={i} style={{ fontSize: 10, color: "#c7d2fe", padding: "2px 0" }}>→ {toStr(g.coverage_gap)} <span style={{ color: "#475569" }}>(missed from concept #{g.id} analysis)</span></div>)}
        </div>
      )}
      {items.map((s, i) => (
        <div key={i} style={{ marginBottom: 10, padding: "8px 10px", borderRadius: 6, background: "rgba(255,255,255,0.02)", borderLeft: `3px solid ${verdictColor(s.verdict)}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: "#e2e8f0" }}>#{s.id} {s.concept}</span>
            <Badge color={verdictColor(s.verdict)}>{s.verdict || "?"}</Badge>
          </div>
          {s.pre_hoc  && <div style={{ marginBottom: 4 }}><div style={{ fontSize: 9, fontWeight: 700, color: "#818cf8", marginBottom: 1 }}>PRE-HOC</div><div style={{ fontSize: 10, color: "#c7d2fe", lineHeight: 1.4 }}>{toStr(s.pre_hoc)}</div></div>}
          {s.post_hoc && <div><div style={{ fontSize: 9, fontWeight: 700, color: "#a78bfa", marginBottom: 1 }}>POST-HOC</div><div style={{ fontSize: 10, color: "#c4b5fd", lineHeight: 1.4 }}>{toStr(s.post_hoc)}</div></div>}
        </div>
      ))}
    </div>
  );
...
function ReportMetaAnalysis({ data, color }) {
  ...
  return (
    <div>
      ...
      {Array.isArray(data.reflexive_observations) && data.reflexive_observations.length > 0 && (
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#f59e0b", marginBottom: 4 }}>🪞 REFLEXIVE OBSERVATIONS</div>
          {data.reflexive_observations.map((r, i) => <div key={i} style={{ fontSize: 10, color: "#fcd34d", padding: "3px 0", lineHeight: 1.5 }}>◈ {toStr(r)}</div>)}
        </div>
      )}
    </div>
  );
...
const RENDERERS = {
  code_preprocess: ReportCodePreprocess,
  dual_extract: ReportDualExtract,
  substantiation: ReportSubstantiation,
...
export default function PhilosophersStone() {
  ...
  const [selected, setSelected] = useState(new Set(STAGES.filter(s => !s.optional).map(s => s.id)));
  ...
  const runPipeline = useCallback(async () => {
    ...
    // ── Pre-flight checks ─────────────────────────────────
    const truncCheck = detectTruncation(input);
    if (truncCheck.truncated) {
      const severity = truncCheck.confidence === "high" ? "❌" : "⚠️";
      addLog(`${severity} Source truncation detected (${truncCheck.confidence} confidence): ${truncCheck.reason} — fidelity gap likely`, "error");
      all.truncated = truncCheck;
    }

    try {
      // ── 0. Code Preprocessor (optional) ──────────────────
      if (selected.has("code_preprocess")) {
        setStatus(p => ({ ...p, code_preprocess: "running" }));
        addLog("🧹 Preprocessing code — extracting semantic payload...");
        try {
          const pr = PROMPTS.code_preprocess(input);
          const cleaned = await callAPI(pr.system, pr.user, 4000);
          pipelineInput = cleaned.trim() || input;
          setResults(p => ({ ...p, code_preprocess: pipelineInput }));
          addLog(`🧹 Preprocessed: ${input.length} chars → ${pipelineInput.length} chars`, "success");
        } catch (e) {
          addLog(`🧹 Preprocessor error: ${e.message} — using raw input`, "error");
        }
        setStatus(p => ({ ...p, code_preprocess: "complete" }));
      }

      const DP = computeDP(pipelineInput);
      addLog(`📐 ${DP.sourceType} source (~${DP.wordCount} words, density ${DP.coefficient}) → ${DP.concepts} concepts · ${DP.terms} terms · ${DP.clusters} clusters`);
      ...
      if (selected.has("dual_extract")) {
        ...
        try {
          const pr = PROMPTS.dual_extract(pipelineInput, DP);
      ...
      // ── 1.5 Substantiation — runs after dual_extract, in parallel with 2+3+4 ──
      const substantiationP = selected.has("substantiation") && all.concepts.length ? (async () => {
        setStatus(p => ({ ...p, substantiation: "running" }));
        addLog("🔎 Substantiating concepts — pre/post hoc rationale...");
        try {
          const pr = PROMPTS.substantiation(pipelineInput, all.concepts);
          const raw = await callAPI(pr.system, pr.user, 6000);
          all.substantiation = extractArray(raw);
          // Demote unsound concepts to inferred: in all.concepts
          const unsound = new Set(all.substantiation.filter(s => s.verdict === "unsound").map(s => s.id));
          if (unsound.size > 0) {
            all.concepts = all.concepts.map(c => {
              if (unsound.has(c.id) && toStr(c.technical).startsWith("extracted:")) {
                return { ...c, technical: toStr(c.technical).replace("extracted:", "inferred:") };
              }
              return c;
            });
            setResults(p => ({ ...p, dual_extract: all.concepts }));
            addLog(`🔎 Demoted ${unsound.size} unsound concept(s) to inferred:`, "error");
          }
          // Surface coverage gaps
          const gaps = all.substantiation.filter(s => s.coverage_gap && s.coverage_gap !== "null");
          if (gaps.length > 0) {
            addLog(`🔎 ${gaps.length} coverage gap(s) detected: ${gaps.map(g => g.coverage_gap).join(", ")}`, "error");
          }
          // Importance score feedback — discount by verdict
          const verdictMultiplier = { sound: 1.0, weak: 0.6, unsound: 0.3 };
          const subById = Object.fromEntries(all.substantiation.map(s => [s.id, s]));
          all.concepts = all.concepts.map(c => {
            const sub = subById[c.id];
            if (!sub) return c;
            const mult = verdictMultiplier[sub.verdict] ?? 1.0;
            const newImportance = Math.round(parseFloat(c.importance || 0) * mult * 100) / 100;
            return { ...c, importance: newImportance };
          });
          setResults(p => ({ ...p, dual_extract: all.concepts, substantiation: all.substantiation }));
          const sound = all.substantiation.filter(s => s.verdict === "sound").length;
          const weak  = all.substantiation.filter(s => s.verdict === "weak").length;
          addLog(`🔎 Substantiation: ${sound} sound · ${weak} weak · ${unsound.size} unsound — importance scores adjusted`, "success");
        } catch (e) {
          addLog(`🔎 Substantiation error: ${e.message}`, "error");
          all.substantiation = [];
        }
        setStatus(p => ({ ...p, substantiation: "complete" }));
      })() : Promise.resolve();

      // ── 2. Substantiation ─────────────────────────────────
      if (selected.has("substantiation") && all.concepts.length) {
        setStatus(p => ({ ...p, substantiation: "running" }));
        addLog("🔎 Substantiating concepts — pre/post hoc rationale...");
        try {
          const pr = PROMPTS.substantiation(pipelineInput, all.concepts);
          ...
          all.concepts = all.concepts.map(c => {
            ...
            if (!sub) return c;
            const mult = verdictMultiplier[sub.verdict] ?? 1.0;
            return { ...c, importance: Math.round(parseFloat(c.importance || 0) * mult * 100) / 100 };
          });
          const gaps = all.substantiation.filter(s => s.coverage_gap && s.coverage_gap !== "null");
          if (gaps.length > 0) addLog(`🔎 ${gaps.length} coverage gap(s): ${gaps.map(g => g.coverage_gap).join(", ")}`, "error");
          setResults(p => ({ ...p, dual_extract: all.concepts, substantiation: all.substantiation }));
          const sound = all.substantiation.filter(s => s.verdict === "sound").length;
          const weak  = all.substantiation.filter(s => s.verdict === "weak").length;
          addLog(`🔎 Substantiation: ${sound} sound · ${weak} weak · ${unsound.size} unsound`, "success");
        } catch (e) {
          addLog(`🔎 Substantiation error: ${e.message}`, "error");
          all.substantiation = [];
        }
      ...
      // ── 3. Triple-Mode ────────────────────────────────────
      if (selected.has("triple_mode") && all.concepts.length) {
        ...
        try {
          ...
          const pr = PROMPTS.triple_mode(pipelineInput, top);
          const raw = await callAPI(pr.system, pr.user, 12000);
          all.elaborations = extractArray(raw);
          const elabRatio = all.elaborations.length / Math.max(Math.max(DP.wordCount, 100) / 50, 1);
          if (elabRatio > 3) addLog(`⚠️ Elaboration inflation: ${all.elaborations.length} elaborations (ratio ${elabRatio.toFixed(1)}x)`, "error");
      ...
      // ── 4. Semantic Index ─────────────────────────────────
      if (selected.has("semantic_index") && all.concepts.length) {
        setStatus(p => ({ ...p, semantic_index: "running" }));
        addLog("🗂️ Building semantic taxonomy...");
        try {
          const pr = PROMPTS.semantic_index(pipelineInput, all.concepts, DP);
      ...
      // ── 5. Compress / Expand ──────────────────────────────
      if (selected.has("compress_expand") && all.concepts.length) {
        setStatus(p => ({ ...p, compress_expand: "running" }));
        addLog("💎 Compressing and expanding...");
        try {
          const compressConcepts = toArr(all.concepts).sort((a, b) => (b.importance || 0) - (a.importance || 0)).slice(0, Math.min(all.concepts.length, 17));
          const pr = PROMPTS.compress_expand(pipelineInput, compressConcepts, DP);
          const raw = await callAPI(pr.system, pr.user, 6000);
      ...
      // ── 5. Meta-Analysis (runs first — feeds prescriptive constraints into forge) ──
      if (selected.has("meta_analysis")) {
        setStatus(p => ({ ...p, meta_analysis: "running" }));
        addLog("📡 Running meta-analysis...");
        try {
          const pr = PROMPTS.meta_analysis(pipelineInput, all, DP);
          const raw = await callAPI(pr.system, pr.user, 4000);
          ...
          // Extract prohibited terms from recommendations for forge constraints
          all.meta._prohibited_terms = toArr(all.meta.recommendations)
            .join(" ")
            .match(/"([^"]+)"/g)?.map(t => t.replace(/"/g, "")) || [];
          if (all.meta._prohibited_terms.length) {
            addLog(`📡 Forge constraints: prohibit [${all.meta._prohibited_terms.join(", ")}]`, "error");
          }
      ...
      // ── 6. Skill Forge (after meta — receives prescriptive constraints) ──
      const forgeP = selected.has("skill_forge") && all.concepts.length ? (async () => {
        ...
        try {
          const pr = PROMPTS.skill_forge(pipelineInput, all, DP);
          const raw = await callAPI(pr.system, pr.user, 4000);
          ...
          const thesis    = toStr(all.compression?.thesis);
          // Pre-compute substantiation summary (can't use IIFE inside template literal)
          const subSummary = (() => {
            const subs = toArr(all.substantiation);
            if (!subs.length) return "_Substantiation not run_";
            const sSound   = subs.filter(s => s.verdict === "sound").length;
            const sWeak    = subs.filter(s => s.verdict === "weak").length;
            const sUnsound = subs.filter(s => s.verdict === "unsound").length;
            const sGaps    = subs.filter(s => s.coverage_gap && s.coverage_gap !== "null");
            let out = `**Sound:** ${sSound} · **Weak:** ${sWeak} · **Unsound (demoted):** ${sUnsound}`;
            if (sGaps.length) out += "\n\n**Coverage Gaps Detected:**\n" + sGaps.map(g => `- 📍 ${toStr(g.coverage_gap)}`).join("\n");
            return out;
          })();
          ...
          const conceptRows = concepts.map(c => {
            ...
            const cite = toStr(c.source_citation).replace(/\|/g, "&#124;");
            return `| ${toStr(c.concept)} | ${toStr(c.technical).replace(/\|/g, "&#124;").slice(0, 200)} | ${toStr(c.plain).replace(/\|/g, "&#124;").slice(0, 200)} | ${imp} | ${cite ? `_"${cite}"_` : "—"} |`;
          }).join("\n");
...
## ${DP.sourceType === "code" ? "Code Implementation" : "Decision Logic"}
${DP.sourceType === "code" ? `\`\`\`python
${forgeData.implementation_code || "# No implementation generated"}
\`\`\`` : (forgeData.implementation_code || "_No decision logic generated_")}
...
${reflexive.length ? reflexive.map(r => `- ◈ ${r}`).join('\n') : '_None detected_'}
## Recommendations
${recs}
...
\`\`\`` : (forgeData.cheat_sheet_code || forgeData.implementation_code || "_No quick reference generated_")}
